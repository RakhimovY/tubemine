...
export const metadata = {
  title: "One quick step before Google - TubeMine",
  description:
    "Read-only YouTube access uses your own personal Google quota. Coming soon, after Google verification.",
}

/*
  TUB-1 Visual Port (Page 7/9): /oauth-intro renders the design HTML
  verbatim (TubeMine OAuth Intro.html). Anonymous-accessible page that
  opts out of the public SiteHeader (handled by SiteHeaderGate) and
  ships its own slim topbar, slim footer, and social row inlined into
  the page markup, so the design's CSS classes (.topbar, .stage,
  .login-card, .oauth-badge, .coming-soon-banner, .google-btn,
  .oauth-skip, .back-link, .trust-mini, .foot-social, .foot) apply
  unchanged. Phase E gate: the Google button is statically disabled
  ("coming soon"); the shared-quota skip-link routes to
  /dashboard?welcome=true so visitors can still use TubeMine via the
  shared YouTube API key while we wait on Google verification.
*/
...
export default async function OauthIntroPage({
  ...
  return (
    <div className="oauth-intro-page">
      <header className="topbar">
        <Link href="/" className="brand" aria-label={t("brand_aria")}>
          <span className="brand-mark" aria-hidden="true" />
          <span>TubeMine</span>
        </Link>
        <div className="topbar-end">
          <DesignLocaleSwitcher languageLabel={t("language_label")} />
          <span className="topbar-meta">{t("version")}</span>
        </div>
      </header>

      <main className="stage">
        <div className="login-wrap">
          <section className="login-card" aria-labelledby="oauth-title">
            <span className="oauth-badge" aria-hidden="true">
              <svg
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
            ...
            <h1 className="login-card-title" id="oauth-title">
              {t("title")}
            </h1>

            <aside className="coming-soon-banner" role="note">
              <span className="dot" aria-hidden="true" />
              <div className="body">
                <span className="title">{t("coming_soon_banner")}</span>
                <span className="sub">{t("coming_soon_sub")}</span>
              </div>
            </aside>

            <p className="login-card-sub">
              {t("helper_lead")} <strong>{t("helper_strong")}</strong>{" "}
              {t("helper_tail")}
            </p>

            <button
              type="button"
              className="google-btn is-disabled"
              id="continueBtn"
            ...
            >
              ...
              <span className="label">{t("continue_disabled")}</span>
            </button>

            <Link href="/dashboard?welcome=true" className="oauth-skip">
              {t("shared_quota_link")}
            </Link>
          </section>
      ...
      <nav className="foot-social" aria-label={t("social_label")}>
        <a
      ...
      <footer className="foot">
        ...
        <div className="links">
          ...
          <a href="mailto:hello@tubemine.app">{t("footer_contact")}</a>
        </div>
      </footer>
    </div>
  )
